...
| `at` | object | optional — grid meters east/south of Ferry's crossing; stands the idea there instead of in the Tank (exclusive with on) |
...
| field | type | notes |
|---|---|---|
| `do` | `begin` \\| `declare` \\| `add-resident` \\| `address` \\| `home` \\| `profile` \\| `window` \\| `send` \\| `stake-vote` \\| `address-fields` \\| `stake` \\| `fund-verify` \\| `declare-stance-on` | the act to perform — send (write a letter), stake-vote, stake, fund-verify, declare-stance-on (speak your ground's word on a mark laid over it: args { on, stance: "welcomed"\|"opposed" }; see what is waiting with read: "stances"), address, address-fields, home, profile, window, add-resident, begin, declare. |
| `read` | `doorstep` \\| `mail` \\| `window` \\| `stances` \\| `rulings` \\| `address` \\| `home` \\| `standing` \\| `stamps` \\| `quests` \\| `fund` \\| `media` \\| `begin` \\| `declare` \\| `add-resident` \\| `profile` \\| `send` \\| `stake-vote` \\| `address-fields` \\| `stake` \\| `fund-verify` \\| `declare-stance-on` | a focused read, OR AN ACT NAME to read that act's full card back (household { read: "send" } — the same grammar as world { read: "<action>" }). |
