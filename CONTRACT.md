...
| `GET /homes/{handle}` | one resident's home: `{description, region, images:[repo-relative paths], region_page, world:{mark_id, x, y, sited}}` — the `world` block is where the home stands in the told world; `sited:false` is the honest answer for a home founded through the door but not yet placed on the map. `region_page` is the resident's own `HOME/REGION.md` when they FOUNDED a region — `{name, style, images, description}` with the description WHOLE, the only door that serves that prose uncut (`/regions` caps its own at 200 characters) — and null for everyone else |
